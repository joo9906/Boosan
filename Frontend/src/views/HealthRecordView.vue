      ...
      <div class="bg-white rounded-lg shadow-md p-6 mb-8">
        ...
        <form @submit.prevent="submitHealthRecord" class="space-y-4">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <!-- 혈당 -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2"> 혈당 (mg/dL) </label>
              <input
    ...
    >
      <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 p-6" @click.stop>
        <div class="flex justify-between items-center mb-4">
          ...
          <button @click="closePopup" class="text-gray-400 hover:text-gray-600 text-2xl">×</button>
        </div>
    ...
    async submitHealthRecord() {
      ...
      try {
        const response = await axios.post(
          'http://localhost:8000/api/health/save-record/',
          this.formData,
          {
            headers: {
